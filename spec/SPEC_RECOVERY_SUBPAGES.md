# SPEC: Recuperación completa de subpáginas con contenido real

## Contexto

Se perdieron subpáginas del sitio durante el desarrollo. Algunas perdieron contenido en la base de datos, otras perdieron el archivo de ruta (dan 404). La estructura base del proyecto (layout público, layout admin, navbar, footer, auth, modelos Prisma) funciona correctamente. El home también funciona.

Este documento contiene TODO el contenido real del colegio ya mapeado a cada sección. NO usar placeholders. Usar exactamente el texto provisto.

---

## TAREA 1: Verificar y crear TODAS las rutas

Revisá `app/(public)/` y asegurate de que TODAS estas rutas existan como `page.tsx`. Si falta alguna, creala. Si existe pero está rota, arreglala.
...
### Patrón para cada page.tsx

Todas las páginas de sección deben seguir este patrón:
...
export default async function PageName() {
  const section = await prisma.section.findUnique({
    where: { slug: 'SLUG_CORRESPONDIENTE' }
  })
    ...
    <main>
      <section className="bg-primary text-white py-16 px-4">
        <div className="max-w-4xl mx-auto">
          <p className="text-sm mb-2 opacity-75">Inicio &gt; Grupo &gt; Sección</p>
      ...
      <section className="max-w-4xl mx-auto py-12 px-4">
        {section?.content ? (
          ...
          <p className="text-gray-500 text-center py-8">Contenido próximamente</p>
        )}
      </section>
    </main>
  )
...
Las páginas de galería (institucional y pastoral) deben tener un layout de grilla de imágenes con lightbox, no solo bloques de texto.

---

## TAREA 2: Actualizar seed.ts con contenido real

Actualizar `prisma/seed.ts` usando `prisma.section.upsert()` para cada sección (clave: slug). A continuación está el contenido EXACTO que debe ir en cada sección, ya estructurado como bloques JSON.

---

### INSTITUCIONAL — Nuestra Escuela
...
**Nota:** Esta página debe renderizar imágenes desde el modelo GalleryImage con album "institucional", con grilla responsive y lightbox.
...
**Nota:** El documento original tiene contenido del primario en la sección secundario. Usar placeholder hasta que el colegio provea la info correcta.
...
**Nota:** Agregar 2 botones de descarga al final de la página: "Formulario de Solicitud de Bonificaciones" y "Declaración Jurada de Solicitud de Becas". Por ahora apuntan a # con texto indicando que el PDF estará disponible próximamente.
...
### PASANTÍAS — Espacios Curriculares
...
### PASANTÍAS — Monitoreo y Evaluación
...
### SECRETARÍAS — Nivel Inicial y Primario
...
**Nota:** Agregar botón de "Formulario Actualización de Cuotas" al final (apunta a # por ahora).

---

### PASTORAL — Información General
**slug:** `pastoral-info`
...
**Nota:** Debe renderizar imágenes desde GalleryImage con album "pastoral", con grilla responsive y lightbox.

---

## TAREA 3: Verificar navegación del Navbar

Todos los links del menú deben apuntar a las rutas correctas:

| Menú | Submenú | Ruta |
|------|---------|------|
| Institucional | Nuestra Escuela | /institucional/nuestra-escuela |
| Institucional | Autoridades | /institucional/autoridades |
| Institucional | Galería | /institucional/galeria |
| Niveles | Inicial | /niveles/inicial |
| Niveles | Primario | /niveles/primario |
| Niveles | Secundario | /niveles/secundario |
| Becas | — | /becas |
| Pasantías | Objetivo | /pasantias/objetivo |
| Pasantías | Espacios Curriculares | /pasantias/espacios-curriculares |
| Pasantías | Lugares | /pasantias/lugares |
| Pasantías | Monitoreo y Evaluación | /pasantias/monitoreo |
| Secretarías | Niv. Inicial y Primario | /secretarias/inicial-primario |
| Secretarías | Niv. Secundario | /secretarias/secundario |
| Administración | — | /administracion |
| Pastoral | Info. General | /pastoral/info |
| Pastoral | Galería | /pastoral/galeria |
| Noticias | — | /noticias |
| Contacto | — | /contacto |

---

## TAREA 4: Estilos consistentes

1. **Header de sección**: Franja color primario, título en blanco, breadcrumb
2. **Contenido**: max-w-4xl centrado, py-12 px-4, tipografía legible
3. **Responsive**: Mobile first
4. **Galerías**: Grilla responsive (1 col mobile, 2 tablet, 3-4 desktop) con lightbox
5. **Botones de descarga** (becas y administración): Estilo primario con icono de descarga

---

## Orden de ejecución

1. Crear todas las rutas/archivos page.tsx faltantes
2. Actualizar seed.ts con TODO el contenido real y correr `npx prisma db seed`
3. Verificar y corregir links del Navbar
4. Asegurar estilos consistentes
5. Navegar CADA subpágina: debe cargar sin 404 y mostrar contenido real
